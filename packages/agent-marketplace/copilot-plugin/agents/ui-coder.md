...
7. Before building or importing a component, check what the design system already exports: consult `packages/design-system/COMPONENTS.md` (generated by `npm run docs:components`) or the `designSystemComponentManifest` export. Many primitives already ship — including wrappers/re-exports around Ark UI (Dialog, Drawer, Tabs, Menu, TreeView, Slider, Select/Combobox, Progress, Field, …) and TanStack Table (`DataTable`). Reuse the design-system export rather than hand-rolling it or importing `@ark-ui/*`, `@tanstack/*`, or `@visx/*` directly; the manifest's `behaviorSource`/`styleOwner` tell you what each export wraps and who owns its styling.
