...
bars) as readily as numbers — both coerce via the same `Number(...)` the
`time` branch uses — but every `xScale(value)` lookup stays on the
ORIGINAL domain entry, since a band scale keyed by `Date` does not resolve a
post-coercion epoch-ms number. The published `end` is the START OF THE NEXT
BAND after the last one selected (estimated from the domain's own spacing),
not that last band's domain value — matching `linear`/`time`, whose
`invert` already lands past the last selected datum, so a half-open
consumer filter (`start <= x < end`, stl's URL schema among them) does not
silently drop the last band the drag visibly covered.
...
cannot support is a domain that is **not numeric even after `Number(...)`
coercion** (e.g. string category labels) — `TimeRange` is
`{ start: number; end: number }`, so no such domain value can produce one.
